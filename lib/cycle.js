...
module.exports = function() {
  ...
  return {
    ...
    loginSuccess: function(req, res, user) {
      ...
      // mak changes because one size don't fit all, homes.
      const queryBasedResponse = req.allParams().responseType;
      const emailValidationRoute = waterlock.config.postActions.validate && waterlock.config.postActions.validate.success;

        // now respond or redirect
      var postResponse = this._resolvePostAction(waterlock.config.postActions.login.success,
        user);

      if (postResponse === 'jwt' || queryBasedResponse === 'jwt') {
          //Returns the token immediately
          var jwtData = waterlock._utils.createJwt( req, res, user );
      ...
      } else if ( queryBasedResponse === 'emailvalidation' && emailValidationRoute ){
        res.redirect( emailValidationRoute );
      } else if(typeof postResponse === 'string' && this._isURI(postResponse)){
        res.redirect(postResponse);
      } else {
        res.ok(postResponse);
